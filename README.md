...
> simulator — no provider calls, no cost. Live calls are opt-in per key.
>
...
> each with its **own monthly spend cap** (`LIVE_CAP_DEFAULT_USD` = $5 until set). A virtual key
> for a configured provider then makes real upstream calls billed to that key, blocked once that
> provider's monthly live spend hits its cap. The admin hands the raw `vk_…` strings to whoever
> needs them — **recipients need no account**. A key with no configured provider stays
> **simulated**. A server env var for a provider configures it for every Workspace and always
...
>
> **Cost**: for **live OpenRouter** calls it's the provider's *actual* charge (`usage.cost`).
> OpenAI/Anthropic/Gemini don't return a per-request cost, so those (and all simulated calls)
> are **estimated** as tokens × a hand-maintained price table. Each row records which:
> `cost_source` = `provider` or `configured`.
...
2. On **Workspace**, **create a virtual key** (allowed providers, optional budget, optionally
   *assign to* a Team Member), copy the `vk_…` string, open **Playground**, paste it, and send a
   request. With `ENABLE_LIVE=false` it runs simulated and shows on the workspace Dashboard and
   Requests.
...
| `ALLOW_LIVE_KEYS` | `true` | Let a Workspace Admin attach the workspace's own provider key to enable live mode. |
...
| `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `OPENROUTER_API_KEY` / `GEMINI_API_KEY` | `""` | Instance-wide provider creds — configure a provider for every workspace. Always win over a workspace's attached key. Never shown in the UI. Gemini uses Google's OpenAI-compatible endpoint. |
| `ENABLE_LIVE` | `false` | Master switch for real upstream calls. When off, every request runs simulated. When on, a request still needs its virtual key flagged `allow_live` **and** a configured provider key, and stops at that provider's monthly cap. |
...
|---|---|---|---|
| GET | `/healthz` | — | health / warmup (`{version, live_enabled}`) |
...
| GET | `/api/requests` | admin | recent request log (`?limit&cursor&provider&model&key_id&status&mode&start&end`) |
...
| **POST** | **`/v1/chat/completions`** | Bearer `vk_…` | **OpenAI-compatible.** `{model:"<provider>/<slug>", messages[], stream?}` → OpenAI `chat.completion` (or SSE chunks). `402` over budget/cap, `403` provider not on key. |
...
- **usage_logs** — `id, key_id, request_id, provider, model, prompt_tokens, completion_tokens,
  ...
  response_preview, ts`. Workspace ownership flows through `key_id → virtual_keys.workspace_id`.
  `cost_source` = `provider` (real charge) or `configured` (tokens × price table). Previews are
  null unless `LOG_BODIES=true`.
- **provider_credentials** — `(id, workspace_id → workspaces (`ON DELETE CASCADE`), provider,
  ...
  workspace's own provider key; its live-flagged virtual keys call real providers on it, and
  `enforce_workspace_cap` blocks live calls on a provider once the calendar month's `mode='live'`
  spend on it hits `monthly_cap_usd` (NULL ⇒ `LIVE_CAP_DEFAULT_USD`). `ciphertext` is a Fernet
